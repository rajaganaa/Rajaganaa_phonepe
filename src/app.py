...
for state in agg_insurance_list:
    ...
    for year in agg_year_list:
        ...
        for file in agg_file_list:
            ...
            # CONVERT JSON FORMAT INTO DATAFRAME
agg_insurance = pd.DataFrame(columns1)
# agg_insurance

# SLICING STATES DETAILS
...
agg_insurance["States"] = agg_insurance["States"].str.replace('Dadra & Nagar Haveli & Daman & Diu',
                                                              'Dadra and Nagar Haveli and Daman and Diu')

# SHOW UNIQUE VALUES OF STATES
# agg_insurance["States"].unique()
# List all files and directories in the specified path
...
aggre_transaction["States"] = aggre_transaction["States"].str.replace('Dadra & Nagar Haveli & Daman & Diu',
                                                                      'Dadra and Nagar Haveli and Daman and Diu')

# List all files and directories in the specified path
agg_user_list = os.listdir(path3)
columns3 = {
    "States": [], "Years": [], "Quarter": [], 'Brand': [],
...
aggree_user = pd.DataFrame(columns3)
# print(aggree_user)
aggree_user["States"] = aggree_user["States"].str.replace('andaman-&-nicobar-islands', 'Andaman & Nicobar')
aggree_user["States"] = aggree_user["States"].str.replace("-", " ")
aggree_user["States"] = aggree_user["States"].str.title()
aggree_user["States"] = aggree_user["States"].str.replace('Dadra & Nagar Haveli & Daman & Diu',
                                                          'Dadra and Nagar Haveli and Daman and Diu')
...
map_insurance["States"] = map_insurance["States"].str.replace('Dadra & Nagar Haveli & Daman & Diu',
                                                              'Dadra and Nagar Haveli and Daman and Diu')
...
map_trans["States"] = map_trans["States"].str.replace('Dadra & Nagar Haveli & Daman & Diu',
                                                      'Dadra and Nagar Haveli and Daman and Diu')
...
map_user["States"] = map_user["States"].str.replace('Dadra & Nagar Haveli & Daman & Diu',
                                                    'Dadra and Nagar Haveli and Daman and Diu')
...
top_insurance["States"] = top_insurance["States"].str.replace('Dadra & Nagar Haveli & Daman & Diu',
                                                              'Dadra and Nagar Haveli and Daman and Diu')
...
top_transaction["States"] = top_transaction["States"].str.replace('Dadra & Nagar Haveli & Daman & Diu',
                                                                  'Dadra and Nagar Haveli and Daman and Diu')
...
top_user["States"] = top_user["States"].str.replace('Dadra & Nagar Haveli & Daman & Diu',
                                                    'Dadra and Nagar Haveli and Daman and Diu')
...
# Create the table if it does not exist
create_query_2 = '''
CREATE TABLE IF NOT EXISTS aggre_transaction(
    States VARCHAR(255),
    Years INT,
    Quarter INT,
    Transaction_type VARCHAR(255),
...
# Create the table if it does not exist
create_query_3 = '''
CREATE TABLE IF NOT EXISTS aggree_userr(
    States VARCHAR(255),
    Years INT,
    Quarter INT,
    Brand VARCHAR(255),
...
insert_query_1 = '''
INSERT INTO agg_insurance (
    States, Years, Quarter, Transaction_type, Transaction_count, Transaction_amount
) VALUES (%s, %s, %s, %s, %s, %s)
'''

data1 = agg_insurance.values.tolist()

# Insert data into the table
insert_query_2 = '''
INSERT INTO aggre_transaction (
    States, Years, Quarter, Transaction_type, Transaction_count, Transaction_amount
) VALUES (%s, %s, %s, %s, %s, %s)
'''

data2 = aggre_transaction.values.tolist()

# Insert data into the table
insert_query_3 = '''
INSERT INTO aggree_userr (
    States, Years, Quarter, Brand, Transaction_count, Percentage
) VALUES (%s, %s, %s, %s, %s, %s)
'''

data3 = aggree_user.values.tolist()
...
data4 = map_insurance.values.tolist()
...
data7 = top_insurance.values.tolist()
...
data8 = top_transaction.values.tolist()
...
data9 = top_user.values.tolist()

try:
    cursor.executemany(insert_query_1, data1)
    cursor.executemany(insert_query_2, data2)
    cursor.executemany(insert_query_3, data3)
    cursor.executemany(insert_query_4, data4)
    cursor.executemany(insert_query_5, data5)
    cursor.executemany(insert_query_6, data6)
    cursor.executemany(insert_query_7, data7)
    cursor.executemany(insert_query_8, data8)
    cursor.executemany(insert_query_9, data9)
...
# AIACY = agg_insurance_amount_count_Y(aggregate_insurance, 2021)
...
# MITACY=Map_ins_transaction_amount_count_Y(map_insurance, 2023)
...
def top_transaction_amount_count_pincode_y(df,states):
    ...
    st.plotly_chart(fig_amount)
    fig_count = px.bar(df_year, x="Quarter", y="Transaction_count", title="TRANSACTION COUNT", hover_data=["pincodes"],
                       color_discrete_sequence=px.colors.sequential.Bluered_r, height=800)
    # fig_count.show()
    st.plotly_chart(fig_count)
